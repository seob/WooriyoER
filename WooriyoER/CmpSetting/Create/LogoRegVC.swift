...
//  PinPle
//
//  Created by WRY_010 on 23/08/2019.
...
class LogoRegVC: UIViewController, NVActivityIndicatorViewable {
    ...
    @IBAction func imgSave(_ sender: UIButton) {
        ...
        IndicatorSetting()
        NetworkManager.shared().Uploadimage(sid: CompanyInfo.sid, image: saveImg, type: 0) { (isSuccess, resCode, resImg) in
            if(isSuccess){
                print("\n---------- [ resCode : \(resCode) ] ----------\n")
                if (resCode == 0){
                    //이미지 업로드 실패
...
extension LogoRegVC : UIImagePickerControllerDelegate,
...
extension LogoRegVC: UIScrollViewDelegate {
