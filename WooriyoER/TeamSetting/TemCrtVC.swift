...
class TemCrtVC: UIViewController, NVActivityIndicatorViewable {
    ...
    @IBAction func create(_ sender: UIButton) {
        ...
        if name != "" {
            IndicatorSetting()
            NetworkManager.shared().RegTeam(temflag: false, cmpsid: cmpsid, name: name.urlEncoding(), memo: memo.urlEncoding(), phone: phone) { (isSuccess, error, result) in
                if (isSuccess) {
                    if error == 1 {
                        let vc = TmCrtSB.instantiateViewController(withIdentifier: "TemCmpltVC") as! TemCmpltVC
                        prefs.setValue(name, forKey: "crt_temname")
                        vc.modalTransitionStyle = .crossDissolve
...
extension TemCrtVC: UITextFieldDelegate {
...
extension TemCrtVC: UITextViewDelegate {
