...
class KACircleCropCutterView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let circle = UIBezierPath(ovalIn: CGRect(x: rect.size.width/2 - setWidth/2, y: rect.size.height/2 - setWidth/2, width: setWidth, height: setWidth))
        context?.setBlendMode(.clear)
        UIColor.clear.setFill()
        circle.fill()
        
        //This is the same rect as the UIScrollView size 240 * 240, remains centered
        let square = UIBezierPath(rect: CGRect(x: rect.size.width/2 - setWidth/2, y: rect.size.height/2 - setWidth/2, width: setWidth, height: setWidth))
