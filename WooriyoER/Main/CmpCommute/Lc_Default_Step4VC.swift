...
class Lc_Default_Step4VC: UIViewController {
    ...
    var anual = "" //연차유급 휴가
    var other = "" // 기타사항
    var payroll = 0 //임금 지급방법(0.직접지급, 1.통장입금)
    var payday = "" 
    ...
    fileprivate func getLCinfo(){
        NetworkManager.shared().get_LCInfo(LCTSID: standInfo.sid) { (isSuccess, resData) in
            if(isSuccess){
                ...
                if serverData.payroll == 0 {
                    // 직접 지급
                    self.directBtn.isSelected = true
                    self.IndirectBtn.isSelected = false
                    self.directBtnImg.image = checkImg
                    self.IndirectBtnImg.image = uncheckImg
                }else{
                    self.IndirectBtn.isSelected = true
                    self.directBtn.isSelected = false
                    self.directBtnImg.image = uncheckImg
                    self.IndirectBtnImg.image = checkImg
                }
            }else{
    ...
    fileprivate func setUi() {
        ...
        case 1:
            formTitle = "표준 계약직 상세"
        case 2:
            formTitle = "표준 시급/소정 상세"
        case 3:
            formTitle = "표준 시급/일별 상세"
        case 4:
            formTitle = "표준 일급/소정 상세"
        case 5:
            formTitle = "표준 일급/일별 상세"
        default:
            break
        }
        ...
        socialinsrn = standInfo.socialinsrn
    ...
    override func setNeedsFocusUpdate() {
        ...
        standInfo.payday = payday
        standInfo.socialinsrn = social
        standInfo.anual = anual
        standInfo.other = other
        standInfo.delivery = delivery
        
        
        
        if standInfo.payroll == 0 {
            // 직접 지급
    ...
    @IBAction func NextDidTap(_ sender: Any) {
        ...
        standInfo.delivery = delivery
        print("\n---------- [ payroll : \(standInfo.payroll) ] ----------\n")
        NetworkManager.shared().lc_std_step4(LCTSID: standInfo.sid, PAYDAY: payday.urlEncoding(), PAYROLL: standInfo.payroll, SOCIAL: social.urlEncoding(), ANUAL: anual.urlEncoding(), DELIVERY: delivery.urlEncoding(), OTHER: other.urlEncoding()) { (isSuccess, resCode) in
            if(isSuccess){
    ...
    @IBAction func moreBtn(_ sender: UIButton) {
        SelLcEmpInfo = standInfo
        SelLcEmpInfo.viewpage = "std_step4"
        if SE_flag {
...
private extension Lc_Default_Step4VC {
...
extension Lc_Default_Step4VC: UITextViewDelegate{
