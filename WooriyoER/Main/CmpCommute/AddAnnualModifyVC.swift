...
class AddAnnualModifyVC: UIViewController {
...
//        minTemp1 = minSave(usemin)
    ...
    @IBAction func saveDidTap(_ sender: UIButton) {
        ...
        print("\n---------- [ changemin : \(changemin) ,  difmin : \(difmin)] ----------\n")
        NetworkManager.shared().udt_Anual(empsid: tempMbrInfo.empsid, anmsid: AnualInfo.sid, type: 1, setmin: changemin, difmin: difmin, memo: memoText.urlEncoding()) { (isSuccess, resCode) in
            if(isSuccess){
                if resCode == 1 {
                    self.toast("수정 되었습니다.")
                    SelEmpInfo.addmin += difmin
                    let vc = CmpCmtSB.instantiateViewController(withIdentifier: "AnnualListVC") as! AnnualListVC
...
extension AddAnnualModifyVC: UITextFieldDelegate {
...
extension AddAnnualModifyVC: UIPickerViewDelegate, UIPickerViewDataSource {
...
extension AddAnnualModifyVC: UITextViewDelegate {
