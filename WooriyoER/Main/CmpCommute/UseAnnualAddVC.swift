...
class UseAnnualAddVC: UIViewController {
    ...
    @IBAction func saveDidTap(_ sender: UIButton) {
        ...
        NetworkManager.shared().ins_Anual(empsid: tempMbrInfo.empsid, type: 0, setmin: use, memo: memoText.urlEncoding()) { (isSuccess, resCode) in
            if(isSuccess){
                if resCode == 1 {
                    self.toast("등록 되었습니다.")
                    SelEmpInfo.usemin += use
                    let vc = CmpCmtSB.instantiateViewController(withIdentifier: "AnnualListVC") as! AnnualListVC
...
extension UseAnnualAddVC: UITextFieldDelegate {
...
extension UseAnnualAddVC: UIPickerViewDelegate, UIPickerViewDataSource {
...
extension UseAnnualAddVC: UITextViewDelegate {
