...
class UseAnnualModifyVC: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        textUseAnlDay.text = timeStr(AnualInfo.setmin)[0].replacingOccurrences(of: "0-", with: "-")
        textUseAnlHour.text = timeStr(AnualInfo.setmin)[1].replacingOccurrences(of: "0-", with: "-")
        textUseAnlMin.text = timeStr(AnualInfo.setmin)[2].replacingOccurrences(of: "0-", with: "-")
        usemin = AnualInfo.setmin
        
        lblDate.text = "수정일 : \(setJoinDate2(timeStamp: AnualInfo.regdt))"
        
        if AnualInfo.memo == "" {
            self.lblMemo.isHidden = false
        }else {
            self.lblMemo.isHidden = true
            textMemo.text = AnualInfo.memo
            let textLength = textMemo.text.count
            lbltextlimit.text = "(\(textLength)/40)"
        }
        
        dayTemp1 = Int(timeStr(usemin)[0]) ?? 0
        dayTemp1 = Int(timeStr(usemin)[0]) ?? 0
        hourTmep1 = Int(timeStr(usemin)[1])  ?? 0
        minTemp1 = Int(timeStr(usemin)[2])  ?? 0
//        minTemp1 = minSave(usemin)
        ...
        useAnlPicker.selectRow(minTemp1, inComponent: 2, animated: false)
    ...
    @IBAction func saveDidTap(_ sender: UIButton) {
        ...
        let changemin = strTime(dayTemp1, hourTmep1, minTemp1)
        let difmin = changemin - usemin
        print("\n---------- [ usemin : \(usemin) ,changemin : \(changemin) ,  difmin : \(difmin)] ----------\n")
        NetworkManager.shared().udt_Anual(empsid: tempMbrInfo.empsid, anmsid: AnualInfo.sid, type: 0, setmin: changemin, difmin: difmin, memo: memoText.urlEncoding()) { (isSuccess, resCode) in
            if(isSuccess){
                if resCode == 1 {
                    self.toast("수정 되었습니다.")
                    SelEmpInfo.usemin += difmin
                    let vc = CmpCmtSB.instantiateViewController(withIdentifier: "AnnualListVC") as! AnnualListVC
                ...
                if resCode == 99 {
                    self.toast("사용한 연차의 시간을 등록해 주세요.")
                }else{
                    self.toast("다시 시도해 주세요.")
                }
            }
    ...
    @IBAction func deleteDidTap(_ sender: UIButton) {
        let vc = CmpCmtSB.instantiateViewController(withIdentifier: "AnnualPopUp") as! AnnualPopUp
        vc.EmpInfo = EmpInfo
        vc.tempMbrInfo = tempMbrInfo
        vc.AnualInfo = AnualInfo
        vc.type = AnualInfo.type
        vc.modalTransitionStyle = .crossDissolve
        vc.modalPresentationStyle = .overFullScreen
        self.present(vc, animated: true, completion: nil)
    }
...
extension UseAnnualModifyVC: UITextFieldDelegate {
...
extension UseAnnualModifyVC: UIPickerViewDelegate, UIPickerViewDataSource {
...
extension UseAnnualModifyVC: UITextViewDelegate {
