...
class CmpsealVC: UIViewController {
    ...
    @objc func fabTapped(_ sender: UIButton){
        faButton.removeFromSuperview()
        var vc = ContractSB.instantiateViewController(withIdentifier: "AddCmpsealVC") as! AddCmpsealVC
        if SE_flag {
            vc = ContractSB.instantiateViewController(withIdentifier: "SE_AddCmpsealVC") as! AddCmpsealVC
        }
...
extension CmpsealVC: UITableViewDataSource , UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "SignTableViewCell", for: indexPath) as? SignTableViewCell {
            ...
            {
                cmpListIndexPath.useflag = 1
                cell.checkedBtn.setImage(CheckimgOn, for: .normal)
            }
            else
            {
                cmpListIndexPath.useflag = 0
                cell.checkedBtn.setImage(CheckimgOff, for: .normal)
            }
             
            cell.signImageView.sd_setImage(with: URL(string: profimg), placeholderImage: UIImage(named: "no_picture"))
    ...
    @objc func checkBoxSelection(_ sender:UIButton)
     {
        ...
        NetworkManager.shared().useCmpseal(cmpsid: userInfo.cmpsid, cslsid: selectedIndexPath) { (isSuccess, resCode) in
            if(isSuccess){
                if resCode == 1 {
                    print("\n---------- [ 성공 ] ----------\n")
                }else{
    ...
    @objc func selInfo(_ sender: UIButton){
        ...
        if selInfo.type == 0 { //직인일때
            var vc = ContractSB.instantiateViewController(withIdentifier: "UdtCmpseal") as! UdtCmpseal
            if SE_flag {
                vc = ContractSB.instantiateViewController(withIdentifier: "SE_UdtCmpseal") as! UdtCmpseal
            }
            ...
            let vc = ContractSB.instantiateViewController(withIdentifier: "UdtCmpsealSign") as! UdtCmpsealSign
            vc.cmpSealInfo = selInfo
            vc.type = selInfo.useflag
