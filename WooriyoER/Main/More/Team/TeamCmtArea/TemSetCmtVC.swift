...
class TemSetCmtVC: UIViewController , CLLocationManagerDelegate{
    ...
    @IBOutlet weak var btnMultiextension: UIButton!
    @IBOutlet weak var swCmpCmt: UISwitch!
    
    @IBOutlet weak var vwHidden: UIView!
    
    @IBOutlet weak var distanceTextField: UITextField!
    
    @IBOutlet weak var vwCompanyArea: UIView!
    @IBOutlet weak var vwGpsArea: UIView!
    @IBOutlet weak var vwWifiArea: UIView!
    @IBOutlet weak var vwBeaconArea: UIView!
    @IBOutlet weak var btnSave: UIButton!
    
    @IBOutlet weak var onvwwifi: UIView!
    @IBOutlet weak var onvwGPS: UIView!
    @IBOutlet weak var onvwwBecon: UIView!
    ...
    let toggleNoti = ToggleSwitch(with: images)
    let toggleCmpCmt = ToggleSwitch(with: images)
...
//        }
        ...
        toggleCmpCmt.frame.origin.x = switchYposition
        toggleCmpCmt.frame.origin.y = 0
        ...
        toggleNoti.frame.origin.x = switchYposition
        toggleNoti.frame.origin.y = 0
        
        self.vwCompanyArea.addSubview(toggleCmpCmt)
        self.vwWifiArea.addSubview(toggleWifi)
        self.vwGpsArea.addSubview(toggleGPS)
        self.vwBeaconArea.addSubview(toggleBeacon)
        self.multiHeaderView.addSubview(toggleNoti)
        
        toggleNoti.addTarget(self, action: #selector(toggleNotiChanged), for: .valueChanged)
        toggleCmpCmt.addTarget(self, action: #selector(toggleCmpCmtChanged), for: .valueChanged)
    ...
    @objc func reloadTableData(_ notification: Notification) {
        cmtarea = SelTemInfo.cmtarea
        wifinm = SelTemInfo.wifinm
        wifimac = SelTemInfo.wifimac
        wifiip = SelTemInfo.wifiip
        loclat = SelTemInfo.loclat
        loclong = SelTemInfo.loclong
        beacon = SelTemInfo.beacon
        locaddr = SelTemInfo.locaddr
        gpsStatus = 1
        CmpinfoUpdate()
    ...
    fileprivate func setUi(){
        ...
        print("\n---------- [ SelTemInfo.locscope : \(SelTemInfo.locscope) ] ----------\n")
        switch SelTemInfo.locscope {
            case 50:
                nSeq = 0
            case 100:
                nSeq = 1
    ...
    // MARK: 회사출퇴근영역 적용 on / off 에 따른 하단 출퇴근 복수 설정영역 노출여부 21.03.04
    @IBAction func setCmpCmt(_ sender: UISwitch) {
        if toggleCmpCmt.isOn {
    ...
    func switchCheck() {
        if toggleWifi.isOn {
            SelTemInfo.cmtarea = 1
            cmtarea = 1
        }else if toggleGPS.isOn {
            SelTemInfo.cmtarea = 2
            cmtarea = 2
        }else if toggleBeacon.isOn {
            SelTemInfo.cmtarea = 3
            cmtarea = 3
        }else if toggleCmpCmt.isOn {
            SelTemInfo.cmtarea = 4
            cmtarea = 4
        }else {
            SelTemInfo.cmtarea = 0
            cmtarea = 0
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if (moreCmpInfo.tmpcmtarea == 99) {
            SelTemInfo.tmpcmtarea = SelTemInfo.cmtarea
            tmpcmtarea = SelTemInfo.tmpcmtarea
            tmpbeacon = SelTemInfo.beacon
            tmpwifinm = SelTemInfo.wifinm
            tmpwifiip = SelTemInfo.wifiip
        }
        
        swSetting(cmtarea)
        lblTname.text = "'\(SelTemInfo.name)' 출퇴근 영역 설정"
        height = multiHeaderView.frame.maxY + 50
        tblListHeightConstraint.constant = height  + 90 + 10
    ...
    func swSetting(_ type: Int) {
        switch type {
            case 0:
                ...
                toggleCmpCmt.setOn(on: false, animated: true)
                toggleWifi.setOn(on: false, animated: true)
                toggleGPS.setOn(on: false, animated: true)
                toggleBeacon.setOn(on: false, animated: true)
            ...
            case 4:
                ...
                toggleCmpCmt.setOn(on: true, animated: true)
                toggleWifi.setOn(on: false, animated: true)
                toggleGPS.setOn(on: false, animated: true)
                toggleBeacon.setOn(on: false, animated: true)
    ...
    func valueSetting() {
        ...
        cmpnm = SelTemInfo.name.urlEncoding()
        cmtarea = SelTemInfo.cmtarea
        beacon = SelTemInfo.beacon
        loclat = SelTemInfo.loclat
        loclong = SelTemInfo.loclong
    ...
    func addcmtareaList(){
        if SelTemFlag {
            NetworkManager.shared().addcmtareaList(cmpsid: 0, ttmsid: SelTtmSid, temsid: 0) { (isSuccess, resData) in
                if(isSuccess){
                    guard let serverData = resData else { return }
            ...
            NetworkManager.shared().addcmtareaList(cmpsid: 0, ttmsid: 0, temsid: SelTemSid) { (isSuccess, resData) in
                if(isSuccess){
                    guard let serverData = resData else { return }
                    if serverData.count > 0 {
                        self.cmtareaList = serverData
    ...
    func updateSetting() {
        ...
        NetworkManager.shared().SetTemCmtarea(temflag: SelTemFlag, ttmsid: SelTtmSid, temsid: SelTemSid, temnm: SelTemInfo.name.urlEncoding(), area: area, wnm: wnm, wmac: wmac, wip: wip, bcon: bcon, lat: lat, long: long, addr: addr, scope: distanceList[nSeq]) { (isSuccess, resultCode) in
            if (isSuccess) {
                if resultCode == 1 {
    ...
    func gpsCheck() -> Bool{
        if (cmtarea == tmpcmtarea &&  wifiip == tmpwifiip && wifinm == tmpwifinm && wifimac == tmpwifimac && beacon == tmpbeacon &&  cmtnoti == tmpcmtnoti && gpsStatus == 0 && SelTemInfo.locscope == distanceList[nSeq]){
            return false
        }else{
            return true
        }
...
extension TemSetCmtVC: UITableViewDelegate , UITableViewDataSource {
...
// MARK: - method
extension TemSetCmtVC {
    @objc func toggleNotiChanged(toggle: ToggleSwitch) {
        statusMulticmtarea()
        if ismulticmtarea {
            if toggle.isOn {
                toast("유효일자(\(moreCmpInfo.multicmtarea))이후 자동으로 비활성화 됩니다.")
    ...
    @objc func toggleCmpCmtChanged(toggle: ToggleSwitch) {
        if toggle.isOn {
            vwHidden.isHidden = false
            swWifi.isOn = false
            swGPS.isOn = false
...
extension TemSetCmtVC: UIPickerViewDelegate, UIPickerViewDataSource{
