...
class AddTtmVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        addToolBar(textFields: [textTName, textTPhone])
        addToolBar(textView: textTMemo)
    ...
    @IBAction func create(_ sender: UIButton) {
        ...
        if name != "" {
            NetworkManager.shared().RegTeam(temflag: true, cmpsid: userInfo.cmpsid, name: name, memo: memo, phone: phone) { (isSuccess, error, resultCode) in
                if (isSuccess) {
                    if error == 1 {
                        switch resultCode {
                        case 0://실패
                        ...
                        default:
                            let vc = MoreSB.instantiateViewController(withIdentifier: "AflTemVC") as! AflTemVC
                            vc.tname = self.textTName.text!
                            vc.ttmsid = resultCode
...
extension AddTtmVC: UITextFieldDelegate {
    //키보드 자동 포커스 이동
...
extension AddTtmVC: UITextViewDelegate {
