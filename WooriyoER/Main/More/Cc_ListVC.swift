...
class Cc_ListVC: UIViewController {
    ...
    fileprivate func searchAction(){
        ...
        lcList.removeAll()
        NetworkManager.shared().searchCc_List(cmpsid: CompanyInfo.sid, curkey: curkey, listcnt: listcnt, name: searchStr.urlEncoding()) { (isSuccess, resnextKey, resData) in
            if(isSuccess){
    ...
    func valueSetting(){
        NetworkManager.shared().get_CcList(cmpsid: CompanyInfo.sid, curkey: curkey, listcnt: listcnt) { (isSuccess, resnextKey, resData) in
            if(isSuccess){
                guard let serverData = resData else { return }
                if serverData.count > 0 {
                    self.nextkey = resnextKey
...
extension Cc_ListVC: UITableViewDataSource , UITableViewDelegate{
...
extension Cc_ListVC : UITextFieldDelegate {
